...
std::vector<Eigen::Array3f>
PrecomputeCubemapSH(const std::vector<std::unique_ptr<float[]>> &images,
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        for (int l = 0; l < SHOrder; l++) {
          for (int m = -l; m <= l; m++) {
            int shindex = sh::GetIndex(l, m);
            double sh = sh::EvalSH(l, m, dir.cast<double>().normalized());
            SHCoeffiecents[shindex] += Le * dw * sh;
          }
        }
      }
    }
...
public:
  ...
  Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1>
  RecursiveBounce(const Point3f &v, const Normal3f &n, const Scene *scene,
                 int bounce, int sample_count) {

    const int sample_side = static_cast<int>(floor(sqrt(sample_count)));
    Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1> shCoeff = Eigen::Matrix<Vector3f::Scalar, SHCoeffLength, 1>::Zero(SHCoeffLength, 1);
    if (bounce == 0)
      return shCoeff;
    ...
    for (int t = 0; t < sample_side; t++) {
      for (int p = 0; p < sample_side; p++) {
        ...
        Ray3f ray(v, wi);
        Intersection its;
        if (cos > 0.003 && scene->rayIntersect(ray, its)) {
          ...
          auto shindirect = bary.x() * sh0 + bary.y() * sh1 + bary.z() * sh2;

          const Mesh *mesh = its.mesh;
          MatrixXf normals = mesh->getVertexNormals();
          Point3f n2 =
              Normal3f(normals.col(its.tri_index.x()).normalized() * bary.x() +
                       normals.col(its.tri_index.y()).normalized() * bary.y() +
                       normals.col(its.tri_index.z()).normalized() * bary.z())
                  .normalized();

          shCoeff +=
              cos * (shindirect +
                     RecursiveBounce(its.p, n2, scene, bounce - 1, sample_count));
        }
      }
    }
  ...
  virtual void preprocess(const Scene *scene) override {
    ...
    for (int i = 0; i < mesh->getVertexCount(); i++) {
      ...
      auto shFunc = [&](double phi, double theta) -> double {
        ...
        if (m_Type == Type::Unshadowed) {
          ...
          // TODO: here you need to calculate shadowed transport term of a
          // given direction
          // TODO: 此处你需要计算给定方向下的shadowed传输项球谐函数值
          double h = n.dot(wi);
          Intersection intersec;
          Ray3f ray(v, wi);
          if (h > 0.0 && !scene->rayIntersect(ray, intersec)) {
            return h;
          }
          return 0;
        }
    ...
    if (m_Type == Type::Interreflection) {
      ...
      for (int i = 0; i < mesh->getVertexCount(); i++) {
        const Point3f &v = mesh->getVertexPositions().col(i);
        const Normal3f &n = mesh->getVertexNormals().col(i);
        auto ret = RecursiveBounce(v, n, scene, m_Bounce, m_SampleCount);
        for (int j = 0; j < SHCoeffLength; j++) {
          indirectCoeffs.col(i).coeffRef(j) = ret.col(0).coeff(j);
        }
      }
